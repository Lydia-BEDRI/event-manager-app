                ...
                return (
                  <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-xl">
                    ...
                    <p className="text-sm mt-1">
                      La capacité totale des zones ({totalZoneCapacity}) est
                      inférieure à la capacité de l'événement (
                      {formData.capacity}). Il reste{" "}
                      {formData.capacity - totalZoneCapacity} places non
                      attribuées.
                    </p>
                  </div>
                );
                ...
                <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-700">
                      Capacité totale des zones:
                    </span>
                    <span className="font-semibold text-blue-900">
                      {getTotalZonesCapacity()}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm mt-2">
                    ...
                    <span
                      className={`font-semibold ${formData.capacity <= getTotalZonesCapacity() ? "text-green-600" : "text-red-600"}`}
                    >
                      {formData.capacity}
                    </span>
                  </div>
                  {zones.length > 0 &&
                    formData.capacity > getTotalZonesCapacity() && (
                      <div className="flex items-center gap-2 text-xs text-red-600 mt-2">
                        <AlertTriangle size={14} className="flex-shrink-0" />
                        <p>
                          La capacité de l'événement doit être ≤ à la somme des
                          zones
                        </p>
                      </div>
                    )}
